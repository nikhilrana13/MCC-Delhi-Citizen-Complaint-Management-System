...
const page = () => {
  ...
  useEffect(()=>{
        const fetchNotifications = async()=>{
          try {
              setLoading(true)
              const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/notification/citizen-notifications`,{
                headers:{
  ...
  // handle mark read notifications
   ...
   const token = localStorage.getItem("token");
      ...
      try { 
          const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/notification/c-markread`,{},{
            headers:{
